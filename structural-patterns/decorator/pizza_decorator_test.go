...
func TestMeat_AddIngredient(t *testing.T) {
	...
	if !strings.Contains(meatResult, "meat") {
		t.Errorf("When calling the add ingredient of the meat decorator it must return a text with the word 'onion', not '%s'", meatResult)
	}
}

func TestPizzaDecorator_MeatAndOnion(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When asking for a pizza with onion and meat the returned string must contain the text '%s' but '%s' didn't have it",
